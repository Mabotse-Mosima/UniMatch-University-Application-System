...
## 4. Assignment 11 — Persistence repository layer (updated diagram)

**Purpose:** Show how domain entities are accessed through **repository interfaces**, with **in-memory** implementations today and **stub** / future backends swappable via a **factory**. Application services use **dependency injection** (constructor) so they depend only on interfaces, not concrete storage.

**Code layout:** `ASSIGNMENT_11/repositories/`, `ASSIGNMENT_11/repositories/inmemory/`, `ASSIGNMENT_11/repositories/stubs/`, `ASSIGNMENT_11/factories/`, `ASSIGNMENT_11/application_services.py`.

*Note:* Nine entity-specific repository interfaces exist (`UserAccountRepository`, `LearnerProfileRepository`, …). The diagram below shows the **pattern** with two entities; the others mirror the same structure.
...
classDiagram
    ...
    class ApplicationRepository {
        <<abstract>>
        +findByLearner(learnerId) List
        +findByProgramme(programmeId) List
    ...
    class DatabaseApplicationRepository {
        -_connectionString String
    }

    class HttpApplicationRepository {
        -_baseUrl String
    }

    class RepositoryFactory {
        <<utility>>
        +createUserRepo(type) UserAccountRepository
        +createApplicationRepo(type) ApplicationRepository
    }

    class ApplicationService {
        -applications ApplicationRepository
        +saveApplication(app) void
        +getApplication(id) Optional
    }
    ...
    UserAccountRepository <|.. FileSystemUserAccountRepository

    ApplicationRepository <|.. InMemoryApplicationRepository
    ApplicationRepository <|.. DatabaseApplicationRepository
    ApplicationRepository <|.. HttpApplicationRepository

    RepositoryFactory ..> InMemoryUserAccountRepository : MEMORY
    RepositoryFactory ..> FileSystemUserAccountRepository : FILE
    RepositoryFactory ..> InMemoryApplicationRepository : MEMORY
    RepositoryFactory ..> DatabaseApplicationRepository : DATABASE
    RepositoryFactory ..> HttpApplicationRepository : HTTP stub

    ApplicationService o-- ApplicationRepository : injects
    UserAccountService o-- UserAccountRepository : injects

    UserAccountRepository ..> UserAccount : persists
    ApplicationRepository ..> Application : persists
```

### Assignment 11 design notes

- **Generic `Repository<T, ID>`** avoids duplicating the four CRUD operations on every entity interface; `ID` is `UUID` for all UniMatch entities in the Python implementation.
- **`RepositoryFactory`** selects the concrete backend (e.g. `MEMORY`, `FILE`, `DATABASE`) at the composition root; services and tests inject the returned interface type.
- **`ApplicationService` / `UserAccountService`** illustrate dependency injection: they only reference repository **interfaces**, so swapping `InMemory*` for file or database implementations does not require changing service code.
